...
def extract_json(text):
    # Try to find JSON object or array in the output text (simple heuristic)
...
def test_meal_plan(goal="lose weight", calories=1800, diet_type="vegetarian"):
    ...
    try:
        ...
        # Use the extraction function here
        ...
        if data is None:
            ...
            if isinstance(data, dict):
                meals = data.get('meals', data)  # fallback to data itself if no 'meals' key
            ...
            if isinstance(meals, list) and meals:
                ...
                save_path = "/content/drive/MyDrive/diet_planner/meal_plan.csv"
                df.to_csv(save_path, index=False)
                print(f"Meal plan saved to {save_path}")
